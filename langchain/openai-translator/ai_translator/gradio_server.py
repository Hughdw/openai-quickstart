...
def launch_gradio():
    iface = gr.Interface(
        ...
        inputs=[
            ...
            gr.Radio(
                [" none", "novel", "news", ""], label="文本类型", info="Where did they go?"
            ),
        ],
...
def launch_gradio_by_blocks():
    with gr.Blocks() as blocks:
        ...
        genre_radio = gr.Radio(["none", "novel", "news", "custom"], label="文体类型", info="选择文体类型，custom 自定义输入", value="none")
        genre_text = gr.Textbox(show_label=False, lines=1, visible=False, placeholder="输入文体类型", value="none")
        ...
        clear = gr.ClearButton(components=[input_file, source_language, target_language, genre_radio, writer_radio, writer_text])
        submit = gr.Button("Submit")

        submit.click(
            translation,
            inputs=[input_file, source_language, target_language, genre_text, writer_text],
            outputs=[output_file],
        )
